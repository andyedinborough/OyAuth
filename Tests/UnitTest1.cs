...
namespace SimpleAuth.Tests {
    ...
    public class UnitTest1 {
        ...
        public void ParseQueryString() {
            var query = SimpleAuth.Utilities.ParseQueryString("http://test/?id=0&id=1&another=test");
            query["id"].Should().Equal("0,1");
            query["another"].Should().Equal("test");

            query = SimpleAuth.Utilities.ParseQueryString(string.Empty, "id=0&id=1&another=test");
            query["id"].Should().Equal("0,1");
            query["another"].Should().Equal("test");
        }
